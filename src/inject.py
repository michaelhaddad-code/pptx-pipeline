...
def _find_shape_span(xml_str: str, shape_id: str) -> Optional[tuple]:
    ...
    candidates = list(shape_open_pattern.finditer(xml_str[:cnvpr_pos + 200]))
...
def _apply_text_autofit(shape_xml: str, target_sz: int) -> str:
    """Set all run font sizes in a shape to a specific target size.

    Used when layout._computed provides a pre-calculated font size
    from compute_text_font_scale(). Only modifies <a:rPr> within <a:r> runs.

    Args:
        shape_xml: raw XML string of the shape
        target_sz: font size in hundredths of a point (e.g. 1400 = 14pt)
    ...
    def _process_run(run_m):
        ...
        content = rpr_pattern.sub(_set_sz, content)
...
def _shrink_font_in_shape_xml(shape_xml: str, original_len: int, new_len: int) -> str:
    ...
    def _scale_rpr(m):
        ...
        if sz_m:
            ...
            # No sz attribute — insert default scaled
            new_sz = max(int(round(_DEFAULT_SZ * scale)), 600)
            attrs = attrs + f' sz="{new_sz}"'
...
def _inject_image_geometry(xml_str: str, shape_id: str, computed: dict,
    ...
    Updates <a:ext cx="..." cy="..."/> and optionally <a:off x="..." y="..."/>
    within the pic shape identified by shape_id.
    ...
    # Update <a:ext cx="..." cy="..."/> — match the xfrm ext (first occurrence in shape)
    ext_pattern = re.compile(
        r'(<[^>]*?:ext\b[^>]*?\bcx\s*=\s*")(\d+)("[^>]*?\bcy\s*=\s*")(\d+)(")'
    )
    new_cx = computed["cx"]
    ...
    shape_xml = ext_pattern.sub(
        lambda m: m.group(1) + str(new_cx) + m.group(3) + str(new_cy) + m.group(5),
        shape_xml, count=1
    )
...
def _inject_images(raw_dir: str, slide_num: int, images: list):
    ...
    for image in images:
        ...
        if compatible_exts is not None and source_ext not in compatible_exts:
            logger.warning("    [WARNING] Format mismatch: replacing %s with %s — may cause display issues", target_ext, source_ext)
    ...
    for shape in shapes_to_inject:
        ...
        if resolved_value is not None:
            ...
            if modified:
                ...
                if computed and computed.get("font_size"):
                    # Use pre-computed font size from layout engine
        ...
        if shape_count > 0:
            ...
            if cnvpr is not None:
                ...
                if shape_node is not None:
                    new_len = _get_shape_text_length(shape_node)

            if new_len > original_len:
                span = _find_shape_span(xml_str, shape_id)
                if span is not None:
                    s, e = span
                    shape_xml = xml_str[s:e]
                    shape_xml = _shrink_font_in_shape_xml(shape_xml, original_len, new_len)
                    xml_str = xml_str[:s] + shape_xml + xml_str[e:]
                    _xml_dirty = True
                logger.info("    [ok] %s (id=%s): %s replacement(s) [font scaled %s->%s chars]", shape_name, shape_id, shape_count, original_len, new_len)
            else:
            ...
            if dynamic_images and not dry_run:
                ...
                if images_with_geometry and os.path.exists(slide_xml_path):
                    ...
                    for img in images_with_geometry:
                        ...
                        # Find original geometry from matching shape in config
                        original_geo = None
